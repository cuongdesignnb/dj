 ...
 */
export default function BookNowError({
  error,
  reset,
  errorMessage,
}: {
  ...
  useEffect(() => {
    if (error) console.error('/book-now render failed', error);
  }, [error]);

  const message = errorMessage ?? 'Something went wrong while loading the booking form.';
