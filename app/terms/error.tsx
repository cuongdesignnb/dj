 ...
 * content: a failed load is shown as a failure, never as local wording that
...
export default function LegalError({
  ...
  useEffect(() => {
    if (error) console.error('support page render failed', error);
  }, [error]);

  const message = errorMessage ?? 'Something went wrong while loading this page.';
