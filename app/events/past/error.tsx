 ...
 * content: a failed load is shown as a failure, never as invented archive
 * entries.
 */
export default function PastEventsError({
  ...
  useEffect(() => {
    if (error) console.error('/events/past render failed', error);
  }, [error]);

  const message = errorMessage ?? 'Something went wrong while loading the event archive.';
