...
/**
 * Skeleton mirroring the /shop layout so nothing jumps when content lands.
 */
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 gap-10 pb-16 pt-10 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            <div className="aspect-[16/12] rounded-[20px] border border-white/[0.06] bg-white/[0.04]" />
          </div>
        </div>

        <div className="grid grid-cols-1 gap-5 py-10 sm:grid-cols-2 lg:grid-cols-3">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-72 rounded-[18px] bg-white/[0.04]" />
          ))}
        </div>
      </Container>
    </main>
