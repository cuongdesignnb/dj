'use client';

import { useRef } from 'react';
import { motion, useInView, useReducedMotion } from 'framer-motion';
import { eventsReveal, eventsStagger } from '@/lib/animations';

interface Props {
  eyebrow?: string;
  title: string;
  /** Lets a section point its aria-labelledby at this heading. */
  titleId?: string;
  context?: string;
  align?: 'left' | 'center';
}

export default function EventsSectionHeading({
  eyebrow,
  title,
  titleId,
  context,
  align = 'left',
}: Props) {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const reduced = useReducedMotion();
  const isCenter = align === 'center';

  return (
    <motion.div
      ref={ref}
      variants={eventsStagger}
      initial="hidden"
      animate={inView ? 'visible' : 'hidden'}
      className={`flex flex-col gap-3 ${isCenter ? 'items-center text-center' : 'items-start text-left'}`}
    >
      <div className={`flex flex-col gap-3 ${isCenter ? 'items-center' : 'items-start'}`}>
        {eyebrow && (
          <motion.span
            variants={eventsReveal}
            className="font-heading uppercase tracking-[0.25em] text-xs sm:text-sm text-rave-red font-semibold"
          >
            {eyebrow}
          </motion.span>
        )}
        <h2
          id={titleId}
          className="font-heading uppercase leading-[1.05] font-black text-white text-3xl sm:text-4xl md:text-5xl"
        >
          {title}
        </h2>
      </div>
      {context && (
        <motion.span
          variants={eventsReveal}
          className="font-heading uppercase tracking-[0.25em] text-xs sm:text-sm text-rave-muted"
        >
          {context}
        </motion.span>
      )}
      {!reduced && (
        <motion.span
          aria-hidden
          variants={eventsReveal}
          className={`block h-[3px] w-20 sm:w-24 mt-1 rounded-full bg-gradient-to-r from-rave-red via-rave-magenta to-rave-red ${
            isCenter ? 'self-center' : 'self-start'
          }`}
          style={{ boxShadow: '0 0 18px rgba(255,23,61,0.55), 0 0 36px rgba(255,23,61,0.3)' }}
        />
      )}
    </motion.div>
  );
}
